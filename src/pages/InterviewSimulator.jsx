...
export function InterviewSimulator() {
  ...
  if (!selectedStack) {
    return (
      <div className="min-h-screen bg-sky-50 font-sans antialiased flex items-center justify-center p-4">
        <div className="max-w-lg w-full space-y-6">
          <div className="text-center">
            <div className="w-16 h-16 bg-gradient-to-br from-cyan-400 to-sky-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg">
              <Code size={28} className="text-white" />
            </div>
            <h1 className="text-2xl font-black text-sky-800">Simulateur d'Entretien</h1>
            <p className="text-sky-500 text-sm mt-1">Choisissez votre stack technique pour un entretien ciblé</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {STACKS.map(s => (
              <button
                ...
                className="bg-white border border-sky-100 hover:border-cyan-300 rounded-2xl p-5 text-left transition-all hover:shadow-md group"
              >
                <span className="text-2xl">{s.icon}</span>
                <p className="font-bold text-sky-800 mt-2 group-hover:text-cyan-600">{s.label}</p>
                <p className="text-xs text-sky-400 mt-0.5">5 questions · Score /20</p>
              </button>
            ))}
          </div>
          <button onClick={() => navigate(-1)} className="w-full py-3 text-sky-500 font-bold text-sm flex items-center justify-center gap-2 hover:text-sky-700">
    ...
    return (
      <div className="min-h-screen bg-sky-50 font-sans antialiased pb-20">
        ...
        <div className="max-w-3xl mx-auto px-4 mt-4 space-y-4">
          {/* Score global */}
          <div className="bg-white rounded-2xl p-6 border border-sky-100 text-center shadow-sm">
          ...
          {/* Détail par question */}
          {results.map((r, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 border border-sky-100 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] font-black text-sky-400 uppercase bg-sky-50 px-2 py-1 rounded-lg">{r.category}</span>
                <span className={`text-sm font-black px-2 py-1 rounded-lg ${getScoreBg(r.score)}`}>
                  {r.score}/{r.maxScore}
                </span>
              </div>
              <p className="text-sm font-bold text-sky-800 mb-3">{r.question}</p>
              <p className="text-xs text-sky-500 bg-sky-50 p-3 rounded-xl italic mb-3">« {r.answer} »</p>
              <div className="flex flex-wrap gap-1.5 mb-3">
                {r.matched.map(kw => (
                  <span key={kw} className="bg-teal-50 text-teal-600 border border-teal-100 px-2 py-0.5 rounded text-[10px] font-bold">
                    ✓ {kw}
                  </span>
                ))}
                {r.missed.map(kw => (
      ...
      <div className="max-w-3xl mx-auto px-4 mt-6 space-y-4">
        <div className="bg-white rounded-2xl p-6 border border-sky-100 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div>
              <span className="text-[10px] font-black text-sky-400 uppercase bg-sky-50 px-2 py-1 rounded-lg">{q.category}</span>
              <p className="text-lg font-bold text-sky-800 mt-2">{q.question}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-sky-100 shadow-sm">
          <h3 className="text-xs font-black text-sky-800 uppercase mb-2 flex items-center gap-2">
